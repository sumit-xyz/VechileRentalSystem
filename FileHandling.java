...
public class FileHandling {
    ...
    public static void main(String[] args) {
        ...
        int idCounter = 1; // Used to assign unique IDs
        String filename = "students.txt"; // File to store student data

        // ======== File Handling - Read from file if exists ========
            ...
            try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (parts.length >= 4) {
                        ...
                        student.put("Roll", parts[2]);
                        student.put("Class", parts[3]);
                        students.add(student);

                        ArrayList<Integer> studentMarks = new ArrayList<>();
                        for (int i = 4; i < parts.length; i++) {
                            studentMarks.add(Integer.parseInt(parts[i]));
                        }
                        marks.put(parts[0], studentMarks);

                        int parsedId = Integer.parseInt(parts[0]);
                        if (parsedId >= idCounter) {
                            idCounter = parsedId + 1;
                        }
                    }
                }
        ...
        while (true) {
            ...
            switch (choice) {
                case 1:
                    ...
                    saveToFile(filename, students, marks);
                    break;
    ...
    public static void saveToFile(String filename, ArrayList<HashMap<String, String>> students, HashMap<String, ArrayList<Integer>> marks) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filename))) {
            for (HashMap<String, String> s : students) {
                String id = s.get("Id");
                StringBuilder line = new StringBuilder(id + "," + s.get("Name") + "," + s.get("Roll") + "," + s.get("Class"));
                ArrayList<Integer> studentMarks = marks.get(id);
